...
set TESTROOT=%~dpf1%
set TESTRESULT=%~dpf1%\testall\primary_vs2022_ms_cmake
...
set CMDLINE="%CMAKE%" --no-warn-unused-cli -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL=TRUE -DRENESAS_INTELLISENSE_HELPER_EXTRA_FLAGS:STRING="-include\"%TESTROOT%/%1/.vs/c_cpp_intellisense_helper.h\"" -DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo -DCMAKE_TOOLCHAIN_FILE:FILEPATH=%TESTROOT%\%1\cmake\%2.cmake -S=%TESTROOT%\%1 -B=%TESTROOT%\%1\build -G Ninja
echo %CMDLINE%
%CMDLINE%
if errorlevel 1 (
    "%CMAKE%" -E sleep 3
...
if not "%~5" == "p,s" goto Ninja_skip_p_s
    set CMDLINE="%CMAKE%" --no-warn-unused-cli -DEXAMPLE_ALT_OUTPUT_TYPE=1 -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL=TRUE -DRENESAS_INTELLISENSE_HELPER_EXTRA_FLAGS:STRING="-include\"%TESTROOT%/%1/.vs/c_cpp_intellisense_helper.h\"" -DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo -DCMAKE_TOOLCHAIN_FILE:FILEPATH=%TESTROOT%\%1\cmake\%2.cmake -S=%TESTROOT%\%1 -B=%TESTROOT%\%1\build -G Ninja
    echo %CMDLINE%
    %CMDLINE%
    set CMDLINE="%CMAKE%" --build %TESTROOT%\%1\build --config RelWithDebInfo --target all --
    echo %CMDLINE%
    ...
    set CMDLINE="%CMAKE%" --no-warn-unused-cli -DEXAMPLE_ALT_OUTPUT_TYPE=2 -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL=TRUE -DRENESAS_INTELLISENSE_HELPER_EXTRA_FLAGS:STRING="-include\"%TESTROOT%/%1/.vs/c_cpp_intellisense_helper.h\"" -DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo -DCMAKE_TOOLCHAIN_FILE:FILEPATH=%TESTROOT%\%1\cmake\%2.cmake -S=%TESTROOT%\%1 -B=%TESTROOT%\%1\build -G Ninja
    echo %CMDLINE%
    %CMDLINE%
    set CMDLINE="%CMAKE%" --build %TESTROOT%\%1\build --config RelWithDebInfo --target all --
    echo %CMDLINE%
...
mkdir %TESTROOT%\%1\build
set CMDLINE="%CMAKE%" --no-warn-unused-cli -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL=TRUE -DRENESAS_INTELLISENSE_HELPER_EXTRA_FLAGS:STRING="-include\"%TESTROOT%/%1/.vs/c_cpp_intellisense_helper.h\"" -DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo -DCMAKE_TOOLCHAIN_FILE:FILEPATH=%TESTROOT%\%1\cmake\%2.cmake -S=%TESTROOT%\%1 -B=%TESTROOT%\%1\build -G "Unix Makefiles"
echo %CMDLINE%
%CMDLINE%
if errorlevel 1 (
    "%CMAKE%" -E sleep 3
